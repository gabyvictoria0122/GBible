...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development gbible npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop gbible
    docker rm gbible
    docker run --name gbible -d --env-file /home/ubuntu/gbible.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/gbible:/dkdata \
        gbible start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./gbible/
...
echo_green "Welcome to gbible's dev env"
